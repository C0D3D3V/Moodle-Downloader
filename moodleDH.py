...
if "My courses" not in contents and "Meine Kurse" not in contents and "Meine Startseite" not in contents:
...
courses = contents.split('class="block_course_list  block list_block"')[1].split('class="footer"')[0]
#>Meine Kurse</h2>

regex = re.compile('<div class="column c1">(.*?)</div>')
...
for course in courses:
    ...
    course_links = soup.find(id="region-main").find_all('a')
    ...
    for link in course_links:
        ...
        if not "else.dhbw-karlsruhe.de" in href:
        ...
        if webfileurl[-4:] == ".php" or webfileurl[-4:] == ".html":
          ...
          for traplink in trap_links:
            ...
            if not "else.dhbw-karlsruhe.de" in hrefT: 
               print(datetime.now().strftime('%H:%M:%S') + " This is an external link. I will store it in the 'externel-links.log' file")
